...
import { Search, Filter, Grid, SlidersHorizontal, MapPin, ArrowRight, Eye, Tag, CheckCircle2, Clock, Ban, Camera, ZoomIn, Send, MessageSquare } from 'lucide-react';
...
export const MemorialLotsPage: React.FC = () => {
  ...
  const getStatusBadge = (st: string) => {
    switch (st) {
      case 'available':
        return <span className="px-2.5 py-1 rounded-full text-[11px] font-bold bg-emerald-100 text-emerald-800 border border-emerald-300 flex items-center gap-1"><CheckCircle2 className="w-3 h-3" /> Available</span>;
      case 'reserved':
        return <span className="px-2.5 py-1 rounded-full text-[11px] font-bold bg-amber-100 text-amber-800 border border-amber-300 flex items-center gap-1"><Clock className="w-3 h-3" /> Reserved</span>;
      case 'occupied':
      case 'full':
        return <span className="px-2.5 py-1 rounded-full text-[11px] font-bold bg-rose-100 text-rose-800 border border-rose-300 flex items-center gap-1"><Ban className="w-3 h-3" /> Occupied</span>;
      default:
        return null;
    }
  };
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-900 font-body pt-24 pb-16">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        {/* Header with thin strip blurry backdrop photo */}
          ...
          <div className="relative z-10 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <div>
              ...
              <p className="text-emerald-100/90 text-xs sm:text-sm mt-1 max-w-2xl font-body">
                Browse lawn plots, family mausoleums, and terrace apartments with real-time status.
              </p>
            </div>
        ...
        {/* Filter Toolbar - Long Emphasized Search Bar & Smaller Compact Filter Dropdowns */}
        <div className="bg-white border border-slate-200 rounded-2xl p-3.5 sm:p-5 mb-8 shadow-sm">
          <div className="flex flex-col lg:flex-row items-stretch lg:items-center gap-3">
            {/* Emphasized Prominent Search Input (Longer & Larger) */}
            ...
            {/* Compact Filter Dropdowns (Smaller & Secondary) */}
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 shrink-0">
              ...
              <div>
                <select
                  value={lotType}
                  onChange={(e) => { setLotType(e.target.value); setPage(1); }}
                  className="w-full bg-slate-100/90 hover:bg-slate-100 border border-slate-200 rounded-lg px-2.5 py-2 text-[11px] font-medium text-slate-700 focus:outline-none focus:border-emerald-600 cursor-pointer"
                >
        ...
        {loading ? (
          ...
          <div className="bg-white border border-slate-200 rounded-2xl p-12 text-center text-slate-500">
            ...
            <h3 className="font-heading italic text-lg text-slate-900 font-bold mb-1">No Memorial Lots Found</h3>
            <p className="text-xs text-slate-500 mb-4">Try clearing filters or searching for another section.</p>
            <button
              onClick={() => {
                setSearch('');
                setSection('ALL');
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {plots.map((plot) => (
              <motion.div
                ...
                className={`bg-white border border-slate-200 rounded-2xl overflow-hidden shadow-sm flex flex-col justify-between group ${plot.status === 'available' ? 'hover:border-emerald-500/55 hover:shadow-md cursor-pointer' : 'opacity-90'}`}
                onClick={() => {
                  if (plot.status === 'available') {
                    navigate(`/lots/${plot.id}`);
                  }
                }}
              >
                <div>
                  {/* Hoverable Cemetery Section Picture Banner */}
                  ...
                  <div className="p-5">
                    {plot.status === 'reserved' && (
                      <div className="mb-3 bg-amber-50 border border-amber-200/90 rounded-xl p-2.5 text-xs text-amber-900 space-y-1.5 shadow-2xs">
                        ...
                        <div className="flex items-center justify-between text-[11px] font-semibold text-amber-800">
                          ...
                          <span className="font-bold text-amber-950">{getGracePeriodText(plot.burial_date)}</span>
                        </div>
                      </div>
                    )}
                    ...
                    <div className="space-y-2 text-xs text-slate-700 mb-2">
                      <div className="flex items-center justify-between py-1 border-b border-slate-100">
                        ...
                        <span className="font-semibold text-emerald-700 capitalize">{plot.lot_type}</span>
                      </div>
                      <div className="flex items-center justify-between py-1 border-b border-slate-100">
                        <span className="text-slate-500">Capacity:</span>
                        <span className="font-semibold text-slate-800">{plot.capacity} Occupants ({plot.current_occupants} current)</span>
                      </div>
                      <div className="flex items-center justify-between py-1 border-b border-slate-100">
                        <span className="text-slate-500">Cemetery:</span>
                        <span className="font-semibold text-slate-800">Solano Public Cemetery</span>
                      </div>
                      <div className="flex items-center justify-between py-1 border-b border-slate-100">
                        <span className="text-slate-500">Location:</span>
                        <span className="font-semibold text-slate-800">Brgy. Curifang, Solano, Nueva Vizcaya</span>
                      </div>
                    </div>
                  </div>
                </div>
                ...
                <div className="px-5 pb-5">
                  <div className="flex items-center justify-between pt-2 border-t border-slate-100 mb-3">
                    <span className="text-xs text-slate-500">Estimated Price:</span>
                  ...
                  {plot.status === 'available' ? (
                    <div className="grid grid-cols-2 gap-2">
                      <button
                        onClick={(e) => {
                          e.stopPropagation();
                          navigate(`/inquiry?plotId=${plot.id}&plotNumber=${plot.plot_number}&section=${plot.section}&lotType=${plot.lot_type}&price=${plot.price}`);
                        }}
                    ...
                    <div className="text-center py-2.5 px-3 bg-slate-100 rounded-xl text-xs font-semibold text-slate-500">
                      {plot.status === 'reserved' ? 'Reserved (Not Available)' : 'Occupied (Not Available)'}
                    </div>
                  )}
                </div>
              </motion.div>
