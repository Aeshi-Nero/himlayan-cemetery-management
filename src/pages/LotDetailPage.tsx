...
import { MapPin, Navigation, ArrowLeft, Send, CheckCircle2, Clock, Ban, ShieldCheck, Heart, Sparkles, ZoomIn, Camera } from 'lucide-react';
...
export const LotDetailPage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const fetchDetail = async () => {
      setLoading(true);
      try {
        const res = await apiClient.get(`/plots/${id}`);
        if (res.data?.success) {
          const currentPlot: Plot = res.data.data;
          setPlot(currentPlot);

          // Fetch similar plots in same section
          const simRes = await apiClient.get(`/plots`, {
            params: { section: currentPlot.section, limit: 3 },
          });
          if (simRes.data?.success) {
            setSimilarPlots(simRes.data.data.filter((p: Plot) => p.id !== currentPlot.id));
          }
        }
      } catch (err) {
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <div className="bg-white border border-slate-200 rounded-2xl overflow-hidden shadow-sm">
              {/* Hoverable Cemetery Grounds Photo Banner */}
              ...
              <div className="p-6">
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                  <div>
                    <h4 className="font-heading font-bold text-slate-900 text-sm">Solano Public Cemetery</h4>
                    <p className="text-xs text-slate-500">Brgy. Curifang, Solano, Nueva Vizcaya</p>
                  </div>
                  <button
                    onClick={() => navigate(`/map?toNode=${plot.nearest_path_node_id || 'node-1'}`)}
            ...
            <div className="bg-white border border-slate-200 rounded-2xl p-6 shadow-sm">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-4 text-xs">
                <div className="bg-slate-50 p-3.5 rounded-xl border border-slate-200">
                  ...
                  <span className="font-bold text-slate-900 text-sm">#{plot.plot_number}</span>
                </div>
                <div className="bg-slate-50 p-3.5 rounded-xl border border-slate-200">
                  <span className="text-slate-500 block mb-1">Section</span>
                  <span className="font-bold text-slate-900 text-sm">Section {plot.section}</span>
                </div>
                <div className="bg-slate-50 p-3.5 rounded-xl border border-slate-200">
                  <span className="text-slate-500 block mb-1">Lot Type</span>
                  <span className="font-bold text-emerald-700 capitalize text-sm">{plot.lot_type}</span>
                </div>
                <div className="bg-slate-50 p-3.5 rounded-xl border border-slate-200">
                  <span className="text-slate-500 block mb-1">Capacity</span>
                  <span className="font-bold text-slate-900 text-sm">{plot.capacity} Occupants</span>
                </div>
                <div className="bg-slate-50 p-3.5 rounded-xl border border-slate-200">
                  <span className="text-slate-500 block mb-1">Occupancy Status</span>
                  <span className="font-bold text-amber-700 text-sm capitalize">{plot.status}</span>
                </div>
                <div className="bg-slate-50 p-3.5 rounded-xl border border-slate-200">
                  <span className="text-slate-500 block mb-1">Cemetery & Location</span>
                  <span className="font-semibold text-slate-800 text-xs">Solano Public Cemetery<br/>Brgy. Curifang, Solano, Nueva Vizcaya</span>
                </div>
              </div>
            </div>
          ...
          <div className="space-y-6">
            <div className="bg-white border border-slate-200 rounded-2xl p-6 shadow-sm">
              ...
              <p className="text-xs text-slate-600 mb-6">
                Reserve or request quotation from our RCC memorial clerk for Lot #{plot.plot_number}.
              </p>
              ...
              {plot.status === 'available' ? (
                ...
                <div className="bg-slate-100 border border-slate-200 rounded-xl p-4 text-center text-xs font-semibold text-slate-600">
                  This memorial lot is currently <span className="capitalize">{plot.status}</span>. Inquiries and reservations are closed.
                </div>
              )}
            </div>
            ...
            {similarPlots.length > 0 && (
              <div className="bg-white border border-slate-200 rounded-2xl p-6 shadow-sm space-y-4">
                <h4 className="text-sm font-heading font-bold text-slate-900">Similar Lots in Section {plot.section}</h4>
                <div className="space-y-3">
                  {similarPlots.map((sim) => (
                    <div
                      key={sim.id}
                      onClick={() => navigate(`/lots/${sim.id}`)}
                    ...
                    >
                      <div>
                        <span className="font-bold text-xs text-slate-900 block">Lot #{sim.plot_number}</span>
                        <span className="text-[11px] text-slate-500 capitalize">{sim.lot_type}</span>
                      </div>
