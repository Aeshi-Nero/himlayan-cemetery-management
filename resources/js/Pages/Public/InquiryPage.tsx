...
import { Send, CheckCircle2, Phone, User, ArrowLeft } from 'lucide-react';

export const InquiryPage: React.FC = () => {
    const { url } = usePage();
    const params = new URL(url, window.location.origin).searchParams;

    const plotIdParam = params.get('plotId');
    const plotNumberParam = params.get('plotNumber');
    const sectionParam = params.get('section');
    const lotTypeParam = params.get('lotType');
    ...
    useEffect(() => {
        ...
        if (plotNumberParam) {
            setMessage(
                `I am interested in inquiring and reserving Memorial Lot #${plotNumberParam} in Section ${sectionParam || 'A'} (${lotTypeParam || 'single'}${
                    priceParam ? `, ₱${Number(priceParam).toLocaleString()}` : ''
                }). Please contact me regarding quotation and payment terms.`,
            );
        }
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            const res = await window.axios.post('/api/inquiries', {
                full_name: fullName,
            ...
            <div className="max-w-3xl mx-auto px-4 sm:px-6">
                <div className="text-center mb-8">
                    <span className="text-xs font-bold uppercase tracking-widest text-emerald-700">
                        Himlayan Public Service
                    </span>
                    <h1 className="text-3xl sm:text-4xl font-heading italic font-bold text-slate-900 mt-1">
                        Submit Memorial Inquiry
                    </h1>
                    <p className="text-slate-600 text-xs sm:text-sm mt-2">
                        Submit your reservation request, plot quotation request, or general inquiry
                        directly to our RCC Memorial Clerks.
                    </p>
                </div>
                ...
                {plotNumberParam ? (
                    ...
                    <div className="mb-6 bg-slate-100 border border-slate-200 rounded-2xl p-4 text-xs text-slate-700 flex items-center justify-between gap-3 shadow-2xs">
                        <span>
                            Notice: Select a specific lot choice from the catalog to submit an
                            inquiry for that lot.
                        </span>
                        <button
                            onClick={() => router.visit('/lots')}
                ...
                {submittedInquiryId ? (
                    <div className="bg-white border border-emerald-200 rounded-2xl p-8 text-center space-y-4 shadow-sm">
                        ...
                        <h2 className="text-2xl font-heading italic font-bold text-slate-900">
                            Inquiry Submitted Successfully
                        </h2>
                        <p className="text-xs text-slate-600 max-w-md mx-auto leading-relaxed">
                            Thank you, <span className="font-bold text-slate-900">{fullName}</span>.
                            Your inquiry reference ID is{' '}
                            <span className="font-mono font-bold text-emerald-700">
                                {submittedInquiryId}
                            </span>
                            . Our RCC Memorial Clerk will review your request for{' '}
                            {plotNumberParam ? `Lot #${plotNumberParam}` : 'your selected lot'} and
                            contact you via {contactNumber}.
                        </p>
                    ...
                    >
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            <div>
                                <label className="block text-xs font-semibold text-slate-700 mb-1">
                                    Requested Burial Date & Time (Optional)
                                </label>
                                <input
                                    type="datetime-local"
                        ...
                        >
                            ...
                            <span>
                                {submitting
                                    ? 'Submitting Inquiry...'
                                    : plotNumberParam
                                      ? `Submit Inquiry for Lot #${plotNumberParam}`
