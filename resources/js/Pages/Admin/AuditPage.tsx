...
export default function AuditPage() {
    ...
    const fetchLogs = async () => {
        setLoading(true);
        try {
            const res = await window.axios.get('/api/audit');
            if (res.data?.success) {
            ...
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                <div>
                    ...
                    <p className="text-xs text-slate-500 mt-1">
                        Real-time activity and access monitoring for RCC Clerk, Cemetery Engineer, and Super Admin
                        user logins.
                    </p>
                </div>
...
AuditPage.layout = (page: ReactNode) => <AdminLayout>{page}</AdminLayout>;
