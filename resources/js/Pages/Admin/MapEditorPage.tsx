...
const createPlotIcon = (plot: Plot, zoomLevel: number = 16) => {
  ...
  if (status === 'occupied' || status === 'full') color = '#ff1744';
...
export default function MapEditorPage() {
  ...
  const handleUpdatePlot = async (e: React.FormEvent) => {
    ...
    setSaving(true);
    try {
      const res = await window.axios.put('/api/plots/' + selectedPlot.id, selectedPlot);
      if (res.data?.success) {
        fetchPlots();
...
MapEditorPage.layout = (page: ReactNode) => <AdminLayout>{page}</AdminLayout>;
