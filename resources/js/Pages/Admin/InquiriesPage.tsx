...
export default function InquiriesPage() {
  ...
  const fetchInquiries = async () => {
    setLoading(true);
    try {
      const res = await window.axios.get('/api/inquiries');
      if (res.data?.success) {
  ...
  const handleStatusChange = async (id: string, status: string) => {
    try {
      const res = await window.axios.patch(`/api/inquiries/${id}`, { status });
      if (res.data?.success) {
  ...
  const handleOpenEdit = (inq: Inquiry) => {
    ...
    setEditStatus(inq.status as 'pending' | 'approved' | 'rejected');
  };
  ...
  const handleEditInquirySubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await window.axios.put(`/api/inquiries/${editingInquiry.id}`, {
  ...
  const handleDeleteInquiry = async (id: string) => {
    try {
      const res = await window.axios.delete(`/api/inquiries/${id}`);
      if (res.data?.success) {
  ...
  const handleConfirmImport = async () => {
    ...
    try {
      const promises = importPreview.map(async (item) => {
        return window.axios.post('/api/inquiries', {
                ...
                {filtered.map((inq) => (
                  <tr key={inq.id} className="hover:bg-slate-50/50 transition-colors">
                        ...
                        <div>
                          ...
                          {inq.created_at && (
                            <span className="block text-[10px] text-slate-400 font-medium mt-0.5">
                              Submitted {new Date(inq.created_at).toLocaleDateString()}
                            </span>
                          )}
                        </div>
                      </div>
                    ...
                    <td className="p-4 text-right pr-6 space-x-1.5 whitespace-nowrap shrink-0">
                      ...
                      <button
                        onClick={() => router.visit('/admin/contracts')}
...
InquiriesPage.layout = (page: ReactNode) => <AdminLayout>{page}</AdminLayout>;
