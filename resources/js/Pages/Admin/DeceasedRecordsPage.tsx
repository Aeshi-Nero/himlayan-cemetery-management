...
export default function DeceasedRecordsPage() {
  ...
  const handleEditBurialSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await window.axios.put(`/api/burials/${editingBurial.id}`, {
  ...
  const handleDeleteBurial = async (id: string) => {
    try {
      const res = await window.axios.delete(`/api/burials/${id}`);
      if (res.data?.success) {
  ...
  const handleConfirmImport = async () => {
    ...
    try {
      const promises = importPreview.map(async (item) => {
        return window.axios.post('/api/burials', {
...
DeceasedRecordsPage.layout = (page: ReactNode) => <AdminLayout>{page}</AdminLayout>;
