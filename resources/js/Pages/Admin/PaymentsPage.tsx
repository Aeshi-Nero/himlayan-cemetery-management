...
export default function PaymentsPage() {
  ...
  const [editAfNumber, setEditAfNumber] = useState('');
  ...
  const fetchPayments = async () => {
    setLoading(true);
    try {
      const res = await window.axios.get('/api/payments');
      if (res.data?.success) {
  ...
  const handleAddPayment = async (e: React.FormEvent) => {
    e.preventDefault();
    try {
      const res = await window.axios.post('/api/payments', {
        contract_id: contractId,
        amount: parseFloat(amount),
        payment_method: paymentMethod,
        receipt_number: orNumber,
        af_51_number: afNumber || undefined,
        notes,
      });
  ...
  const handleOpenEdit = (p: Payment) => {
    ...
    setEditAfNumber(p.af_51_number || '');
  };
  ...
  const handleEditPaymentSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await window.axios.put(`/api/payments/${editingPayment.id}`, {
        amount: parseFloat(editAmount),
        payment_method: editPaymentMethod,
        receipt_number: editOrNumber,
        af_51_number: editAfNumber || undefined,
        notes: editNotes,
      });
  ...
  const handleDeletePayment = async (id: string) => {
    try {
      const res = await window.axios.delete(`/api/payments/${id}`);
      if (res.data?.success) {
  ...
  const handleConfirmImport = async () => {
    ...
    try {
      const promises = importPreview.map(async (item) => {
        return window.axios.post('/api/payments', {
          ...
          <div className="bg-white border border-slate-200 rounded-3xl max-w-md w-full p-6 space-y-4 text-slate-900 shadow-2xl relative">
            ...
            <form onSubmit={handleAddPayment} className="space-y-4 text-xs font-semibold">
              ...
              <div>
                ...
                >
                  {contracts
                    .filter((c) => ['rental_computed', 'paid', 'pending_approval'].includes(c.status))
                    .map((c) => (
              ...
              {selectedContract?.payment_type === 'installment' && (selectedContract.installment_schedules?.length ?? 0) > 0 && (
                <div className="border border-slate-200 rounded-xl bg-slate-50/60 p-3 space-y-2">
                  <div className="flex items-center justify-between">
                    <span className="text-[10px] font-bold text-slate-500 uppercase tracking-wider">Installment Schedule</span>
                    <span className="text-[10px] font-extrabold text-emerald-700">
                      Bal: ₱{selectedContract.balance_remaining?.toLocaleString()}
                    </span>
                  </div>
                  <div className="max-h-32 overflow-y-auto space-y-1.5">
                    {(selectedContract.installment_schedules ?? []).map((s) => (
                      <div key={s.id} className="flex items-center justify-between text-[10px] bg-white border border-slate-200 rounded-lg px-2.5 py-1.5">
                        <span className="font-mono text-slate-700">
                          {new Date(s.due_date).toLocaleDateString()}
                          <span className="ml-2 text-slate-400 font-medium">₱{s.amount_due?.toLocaleString()} / ₱{s.amount_paid?.toLocaleString()}</span>
                        </span>
                        <span className={`px-1.5 py-0.5 rounded font-extrabold uppercase text-[9px] border ${statusBadgeClass(s.status)}`}>
                          {s.status}
                        </span>
                      </div>
                    ))}
              ...
              <div>
                ...
                <input
                  ...
                  placeholder="Optional official AF-51 reference"
            ...
            <form onSubmit={handleEditPaymentSubmit} className="space-y-4 text-xs font-semibold">
              ...
              <div>
                ...
                <input
                  type="text"
                  value={editAfNumber}
                  onChange={(e) => setEditAfNumber(e.target.value)}
                  className="w-full bg-slate-50 border border-slate-200 rounded-xl px-3.5 py-2.5 font-mono text-slate-900"
                />
              </div>
...
PaymentsPage.layout = (page: ReactNode) => <AdminLayout>{page}</AdminLayout>;
