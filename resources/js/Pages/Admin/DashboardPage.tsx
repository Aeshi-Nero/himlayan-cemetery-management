    ...
    const fetchDashboard = async () => {
      try {
        const [dashRes, inqRes, burRes] = await Promise.all([
          ...
          user?.role === 'rcc' ? window.axios.get('/api/inquiries') : Promise.resolve({ data: { success: false } }),
          user?.role === 'rcc' ? window.axios.get('/api/burials') : Promise.resolve({ data: { success: false } })
        ]);
        ...
        <>
          ...
          <div className="space-y-3">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              {/* Action 1 */}
              <button
                onClick={() => router.visit('/admin/burials')}
              ...
              {/* Action 2 */}
              <button
                onClick={() => router.visit('/admin/inquiries')}
                className="group relative bg-white border border-slate-200/80 rounded-2xl p-5 text-left transition-all hover:shadow-md hover:border-emerald-600/30 cursor-pointer"
              >
                <div className="flex items-center gap-4">
                          ...
                          <div key={inq.id} className="p-4 rounded-xl border border-slate-100 bg-slate-50/50 hover:bg-slate-50 transition-colors flex flex-col md:flex-row md:items-center justify-between gap-4">
                            <div className="space-y-1.5 max-w-2xl">
                              <div className="flex items-center gap-2 flex-wrap">
                                ...
                                <span className="text-[10px] text-slate-400">•</span>
                                <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">{new Date(inq.created_at || inq.inquiry_date).toLocaleDateString()}</span>
                                <span className="text-[10px] text-slate-400">•</span>
                ...
                <tbody className="divide-y divide-slate-100">
                  {stats?.recentActivity?.slice(0, 5).map((log: ActivityLog) => (
                    <tr key={log.id} className="hover:bg-slate-50/80 transition-colors">
                      <td className="p-3 font-mono text-[10px] text-slate-400">{log.created_at ? new Date(log.created_at).toLocaleString() : ''}</td>
