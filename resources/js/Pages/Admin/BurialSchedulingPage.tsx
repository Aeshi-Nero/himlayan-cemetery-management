...
export default function BurialSchedulingPage() {
  ...
  const fetchBurials = async () => {
    setLoading(true);
    try {
      const res = await window.axios.get('/api/burials');
      if (res.data?.success) {
  ...
  const fetchPlots = async () => {
    try {
      const res = await window.axios.get('/api/plots');
      if (res.data?.success) {
  ...
  const handleAddBurial = async (e: React.FormEvent) => {
    e.preventDefault();
    try {
      const res = await window.axios.post('/api/burials', {
  ...
  const handleUpdateStatus = async (burialId: string, newStatus: 'completed' | 'cancelled') => {
    ...
    try {
      const res = await window.axios.put(`/api/burials/${burialId}`, {
  ...
  const handleApproveBurial = async (burialId: string) => {
    setUpdatingStatusId(burialId);
    try {
      const res = await window.axios.post(`/api/burials/${burialId}/approve`);
      if (res.data?.success) {
        setApproveConfirmId(null);
        fetchBurials();
        showToast('success', 'Burial approved successfully.');
      }
    } catch (err) {
      console.error('Error approving burial:', err);
      showToast('error', 'Failed to approve burial.');
    } finally {
      setUpdatingStatusId(null);
    }
  };
  ...
  return (
    <div className="space-y-6">
                        ...
                        <td className="p-4 font-mono text-slate-600">
                          {b.approved_at ? (
                            <>
                              ...
                              <div className="text-[10px] text-slate-400 mt-0.5">
                                {new Date(b.approved_at).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}
                              </div>
                            </>
                          ) : (
                        ...
                        <td className="p-4 text-right pr-6">
                          {b.burial_status === 'scheduled' && (
                            <div className="flex items-center justify-end gap-2">
                              <button
                                disabled={updatingStatusId !== null}
                                onClick={() => setApproveConfirmId(b.id)}
                                className="bg-white hover:bg-emerald-50 text-emerald-700 border border-emerald-200 hover:border-emerald-300 font-bold py-1.5 px-3 rounded-lg text-[10px] uppercase tracking-wider transition-colors cursor-pointer flex items-center gap-1"
                              >
      ...
      {approveConfirmId && (() => {
        const pendingBurial = burials.find((b) => b.id === approveConfirmId);
        return (
          <div className="fixed inset-0 z-50 bg-black/40 backdrop-blur-sm flex items-center justify-center p-4">
            <div className="bg-white border border-slate-200 rounded-3xl max-w-sm w-full p-6 space-y-4 text-slate-900 shadow-2xl relative text-center">
              ...
              <div className="space-y-1">
                <h3 className="text-base font-bold text-slate-900">Approve Burial?</h3>
                <p className="text-xs text-slate-500 font-medium">
                  This will mark {pendingBurial?.deceased_name || 'this burial'} as completed and record the approval timestamp.
                </p>
              </div>
              ...
              <div className="flex gap-2.5 pt-2">
                ...
                <button
                  disabled={updatingStatusId !== null}
                  onClick={() => handleApproveBurial(approveConfirmId)}
                  className="w-1/2 bg-emerald-600 hover:bg-emerald-500 text-white font-bold py-2 rounded-xl text-xs transition-all cursor-pointer active:scale-95"
                >
                  {updatingStatusId === approveConfirmId ? 'Approving...' : 'Approve Burial'}
                </button>
              </div>
            </div>
          </div>
...
BurialSchedulingPage.layout = (page: ReactNode) => <AdminLayout>{page}</AdminLayout>;
